...
const parse_listing_file = async (
	file_path: string
): Promise<ProductInfo[]> => {
...
const parse_all_listing_pages = async (): Promise<void> => {
	...
	const file_paths = html_files.map((file) =>
		path.join(listing_pages_dir, file)
	)
	...
	const parse_promises = file_paths.map((file_path) =>
		parse_listing_file(file_path)
	)
	...
	const unique_products: ProductInfo[] = Array.from(products_map.entries()).map(
		([url, title]) => ({
			url,
			title,
		})
	...
	await fs.writeFile(
		output_path,
		JSON.stringify(unique_products, null, 2),
		'utf-8'
	)
